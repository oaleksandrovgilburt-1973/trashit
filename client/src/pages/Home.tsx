...
export default function Home() {
  ...
  const { data: nextVisit } = trpc.subscriptions.myNextVisit.useQuery(undefined, {
    enabled: !!user,
  });
...
<div className="grid grid-cols-2 gap-3">
          ...
          {!user && !loading && (
            <div className="mt-6 p-5 rounded-2xl bg-secondary border border-border text-center">
              ...
              <div className="grid grid-cols-2 gap-2 max-w-sm mx-auto">
                <Link href="/auth">
                  <button className="w-full flex items-center justify-center gap-1.5 trashit-btn-primary text-sm whitespace-nowrap px-2">
                    <LogIn className="w-4 h-4 flex-shrink-0" />
                    {t.login}
                  </button>
                </Link>
                <Link href="/auth">
                  <button className="w-full flex items-center justify-center gap-1.5 trashit-btn-outline text-sm whitespace-nowrap px-2">
        ...
        <div className="container py-6 text-center">
          <Link href="/terms" className="text-xs text-muted-foreground underline hover:text-foreground transition-colors">
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-border w-full max-w-sm pointer-events-auto animate-in slide-in-from-top-4 duration-300">
            ...
            <div className="px-4 pb-4 space-y-2 text-sm text-muted-foreground">
              <ul className="space-y-2 pl-2">
                ...
                <li>♻️ <strong>{isBg ? "Разделно събиране" : "Recycling Collection"}</strong> — {isBg ? "До 3 чувала по 15л. до ~4 кг общо, разделени по вид. Вземаме го от входната Ви врата до контейнера. 1 кредит = 3 чувала. Заявява се с " : "Up to 3 bags of 15L up to ~4 kg total, sorted by type. Collected from your door to the bin. 1 credit = 3 bags. Order with "}<Link href="/credits" onClick={closeServicesPopup} className="text-primary underline">{isBg ? "кредит" : "credits"}</Link> {isBg ? "или" : "or"} <Link href="/subscription" onClick={closeServicesPopup} className="text-primary underline">{isBg ? "абонамент" : "subscription"}</Link>.</li>
